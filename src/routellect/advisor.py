...
class Advisor:
    ...
    def recommend(self, request: RecommendationRequest) -> RecommendationResponse:
        ...
        ranked = sorted(candidates, key=lambda candidate: (-candidate.score, candidate.cost))
        ...
        for raw, quality, cost, latency in eligible:
            ...
            contributions = (
                ...
                ScoreContribution(
                    ...
                    explanation=f"Relative cost utility contributes {cost_contribution:+.3f}.",
                ),
                ScoreContribution(
                    key="latency",
                    ...
                    explanation=(
                        f"Relative latency utility contributes {latency_contribution:+.3f}."
                    ),
                ),
                ScoreContribution(
                    key="evidence_risk",
    ...
    def _useful_shortlist(
        ...
        if remaining:
            economical = min(remaining, key=lambda item: (item.cost, -item.quality))
            flags = ("lowest_cost",)
        ...
        if remaining:
            specialist = max(all_ranked, key=lambda item: (item.quality, item.score))
            if all(item.raw is not specialist.raw for item in selected):
                ...
                alternative = min(
                    local or remaining, key=lambda item: (item.latency, -item.quality)
                )
                flags = ("local_or_fast",)
