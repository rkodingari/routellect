...
TASK_TERMS: dict[str, tuple[WeightedTerm, ...]] = {
    "code": tuple(
...
def _weighted_scores(text: str) -> tuple[dict[str, float], int]:
    ...
    negated_matches = 0
    for label, terms in TASK_TERMS.items():
        score = 0.0
...
def _capabilities(text: str) -> tuple[set[str], int]:
    capabilities = {"text"}
    negated_matches = 0
...
def deterministic_profile(text: str, mode: AssessorMode) -> PromptProfile:
    ...
    capabilities, capability_negations = _capabilities(normalized)
    ...
    uncertainty = 0.10
    reasons: list[str] = ["boundary_lexical_profile"]
    if best_score == 0:
        uncertainty += 0.50
    ...
    privacy_flags = [
        name for name, pattern in SECRET_PATTERNS.items() if pattern.search(normalized)
    ]
    if privacy_flags:
    ...
    return PromptProfile(
        ...
        assessor=AssessorReport(
            ...
            assessor_version="deterministic-profiler-unified-v1",
        ),
    )
